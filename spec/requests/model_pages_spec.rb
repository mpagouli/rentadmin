...
describe "ModelPages" do
  ...
  describe "Models:" do
	...
	#  		visit models_path
	#  	end
	#    #it { should have_link "Models", href: '/models' }
	#    it { should have_selector('title', text: full_title('Models')) }
	#    it { should have_link('Help', href: '#' ) }
	#    it { should have_link('Sign out', href: signout_path ) }
	#  end
	#  describe "List" do
	#    before do  
	#      5.times { FactoryGirl.create(:make) }
	#      5.times { FactoryGirl.create(:group) }
	#      groups = Group.all
	#      Make.all.each_with_index do |mak,i|
	#      	7.times { FactoryGirl.create(:model, make: mak, group: groups[i]) }
	#      end
	#      sign_in admin
	#      visit models_path
	#    end
	...
	#    end
	#    #it { should have_link "Models", href: '/models' }
	#    context "'New Model' button" do 
	#      it { should have_selector('form', :action => '/models/new') }
	#      it { should have_selector('input', :type => 'submit', :value => 'New Model') }
	#    end
	#    context "pagination" do
	#      it { should have_selector('div.pagination') }
	#      it ": should list each model" do
	#        Model.paginate(page: 1).each do |mod|
	#          page.should have_selector('div', text: "#{mod.model_name}")
	#        end
	#      end
	#    end
	#    describe "delete links" do
	#      it { should have_link('delete', href: model_path(Model.first)) }
	#      it "should work if model not referred by another resource" do 
	#        expect { click_link('delete') }.to change(Model, :count).by(-1)
	#      end
	#      it "should not work if model referred by another resource" do 
	#      	@vehicle = Vehicle.new(reg_no: 'AXB 34567')
	#      	@vehicle.model = Model.first
	#      	@vehicle.save
	#        expect { click_link('delete') }.not_to change(Model, :count).by(-1)
	#      end
	#      it "should redirect if model referred by another resource" do 
	#      	@vehicle = Vehicle.new(reg_no: 'AXB 34567')
	#      	@vehicle.model = Model.first
	#      	@vehicle.save
	#        expect { click_link('delete') }.to redirect_to(model_path(Model.first))
	#      end
	#    end
